...
你可以在运行时控制通用查询日志和慢查询日志。你可以开启或禁用日志，或改变日志文件名。你可以告诉服务器将通用查询日志和慢查询日志写入日志表、日志文件或两者都写入。详细信息见5.4.1节 选择通用查询日志和慢查询日志输出目的地，5.4.3节 通用查询日志，5，4，5节 慢查询日志。
...
### 5.4.1 选择通用查询日志和慢查询日志输出目的地（to be continued）
...
如果日志启用了，MySQL服务器可以灵活地控制写入通用查询日志和慢查询日志的输出目的地。日志可能的目的地是日志文件，或`mysql`系统数据库中的`general_log`和`slow_log`表。文件输出、表输出或两者都可以被选择。
...
* 标识事件发生的客户端：
	...
	* host
	客户端主机
	* thread
	mysqld负责产生错误事件的线程ID。该ID指示服务器的哪个部分产生了该事件，与通用查询日志和慢查询日志消息（包括连接线程ID）一致。
	* query_id
	查询ID
* 调试信息：
	* source_file
...
#### 5.4.2.8 将错误记录到系统日志（不明白）

mysqld也可以将错误记录到系统日志（Windows上的事件日志，或Unix和类Unix上的syslog）。

本节介绍如何使用内建过滤器`log_filter_internal`和系统日志接收器`log_sink_syseventlog`来设置错误日志，以立即生效并在后续的重启中也生效。关于如何配置错误日志的一般信息，见5.4.2.1节 错误日志配置。

要启用系统日志接收器，首先加载该接收器组件，然后修改`log_error_services`的值：
```sql
INSTALL COMPONENT 'file://component_log_sink_syseventlog';
...
>Note
>
对于MySQL8.0的设置，你必须显式启用将错误记录到系统日志中。这与MySQL5.7及更早的版本不一样，早期版本在Windows平台上错误记录到系统日志是默认开启的，并且在所有平台上都不需要加载组件。
>
错误记录到系统日志可能需要额外的系统配置，查阅你品台的系统日志文档。

在Windows上，写入应用程序中的事件日志的错误消息有以下特点：
* 标记为`Error, Warning, Note`的条目写入事件日志，但来自单个存储引擎的诸如信息语句的消息不会写入。
* 事件日志条目有个内容为`MySQL`的来源字段。（如果`syseventlog.tag`被定义为tag，则来源为`MySQL-tag`）

在Unix和类Unix系统上，使用syslog记录到系统日志。下面的系统变量会影响syslog消息：
* syseventlog.facility：syslog消息的默认facility是daemon。设置此变量以指定其他facility。
* syseventlog.include_pid：是否在syslog输出的每一行中包括服务器进程ID 。
* syseventlog.tag：此变量定义了一个标签，该标签将添加到syslog消息中的服务器标识符（mysqld）中。如果定义了该标签，则标签将以前导连字号附加到标识符。
>Note
>
>在MySQL8.0.13以前，以上三个变量为log_syslog_facility、log_syslog_include_pid、log_syslog_tag，而不是syseventlog.xxx。

MySQL将用户标签（custom label）“System”用于重要的系统信息，比如正常状态下的重启，关机，以及一些重要的设置改变。在不支持用户标签的日志中，包括Windows的事件日志、Unix和类Unix的syslog，会为系统消息分配用于信息优先级的标签。但是，即使MySQL的log_error_verbosity设置排除信息级别的消息，这些消息也会打印到日志中 。

当日志接收器必须以这种方式退回到“信息”而不是“系统”的标签 ，并且日志事件在MySQL服务器之外进一步处理（例如，由syslog配置过滤或转发 ）时，这些事件可能会默认情况下，辅助应用程序将其作为 “信息”优先级而不是 “系统”优先级进行处理。

#### 5.4.2.9 错误日志输出格式

每个错误日志接收器（写入器）组件都有一个独特的输出格式，用于将消息写入目的地，但其他因素也可能会影响消息的内容：
* 日志接收器可用的信息。如果一个日志接收器执行之前，一个日志过滤器组件先执行了并移除了一个日志事件字段，则该字段就不可用于写入。关于日志过滤的信息，见5.4.2.4节 错误日志过滤类型。
* 与日志接收器相关的信息。并非每个接收器都写入错误事件可用的所有字段。
* 系统变量可能会影响日志接收器。

关于错误事件字段名及其介绍，见5.4.2.3节 错误事件字段。对于所有的日志接收器，错误日志消息中的线程ID，就是mysqld中负责写消息的线程。该ID表明了服务器的哪一部分产生了该消息，并且与通用查询日志和慢查询日志消息一致，包含连接线程ID。

*5.4.2.9.1 log_sink_internal输出格式*

*5.4.2.9.2 log_sink_json输出格式*

*5.4.2.9.3 log_sink_syseventlog输出格式*

*5.4.2.9.4 启动早期日志输出格式*

*5.4.2.9.5 影响错误日志格式的系统变量*

##### 5.4.2.9.1 log_sink_internal输出格式

内部日志接收器产生传统的错误日志输出，比如：
```
...
```
“[”和“]”方括号字符是消息格式的文本字符。他们不表示字段是可选的。

`label`字段的值与错误事件优先级字段`prio`的字符串相对应。

`[err_code], [subsystem]`字段是MySQL8.0加入的。他们是旧版本服务器生成的日志中所缺少的。~~日志解析器可以将这些字段视为消息文本的一部分，仅对于最近才由服务器写入的日志（包括它们）才存在。~~日志解析器必须将err_code视为字符串值，不能是数字，因为该值比如MY_012487还包含非数字字符。

##### 5.4.2.9.2 log_sink_json输出格式

JSON格式的日志接收器产生的消息跟JSON对象一样，包含键值对。比如：
```
{
  "prio": 3,
  "err_code": 10051,
...
```
显示的消息进行了重新编排以提高可读性。写入错误日志中的事件每行显示一条消息。

`ts`（timestamp）键是在MySQL8.0.20加入的，对于JSON格式的日志接收器是唯一的。其值是一个整数值，指示自纪元('1970-01-01 00:00:00' UTC)以来的毫秒数。

`ts`和`buffered`的值时Unix时间戳值，可以使用`FROM_UNIXTIME()`函数和适当的除数进行转换：
```sql
...
系统日志接收器产生的输出格式遵从本地平台使用的系统日志的格式。

##### 5.4.2.9.4 启动早期日志输出格式

服务器在处理启动选项之前，也就是说在服务器知道`log_error_verbosity`和`log_timestamps`系统变量的设置之前，以及在它知道用哪些日志组件之前，服务器就会生成一些错误日志消息。服务器处理启动早期的错误日志消息按照如下方式：
* 在MySQL8.0.14之前，服务器会生成具有默认时间戳、格式以及赘言标签的消息，并缓存他们。在处理启动选项并知道错误日志配置之后，服务器会刷新缓存的消息。由于这些早期的消息用的是默认的日志配置，他们可能与启动选项指定的消息有所不同。此外，早期消息不会刷新到除默认接收器意外的接收器。比如，记录到JSON接收器的消息不会包含早期的消息，因为早期的消息不是JSON格式的（因为默认的不是JSON格式，早期的消息都是默认格式）。
* 在MySQL8.0.14之后，服务器缓存日志事件，而不是缓存格式化的日志消息。这样，它就可以在知道配置后，将配置追溯应用到那些早期事件，这就可以使用配置过的设置刷新消息，而不是使用默认设置。同样，消息将刷新到所有已配置的接收器，而不仅仅是默认接收器。  
如果在知道日志配置之前，服务器发生了致命的错误必须退出，则服务器会使用日志默认的设置格式化缓存的消息，以确保不会丢失。如果没有知名错误发生，但在处理启动选项之前启动速度过慢，则服务器会使用日志默认的设置定期格式化和刷新缓存的消息，以免显得无响应。在使用默认设置的情况下，尽管这种行为与之前的MySQL8.0.14的行为相似，但是当意外情况发生时，最好丢弃消息。

##### 5.4.2.9.5 影响错误日志格式的系统变量

`lig_timestamps`系统变量控制写入错误日志消息时间戳的时区（对于通用查询日志和慢查询日志文件也是）。在到达任何日志接收器之前，服务器将`log_timestamps`应用于错误事件；因此，它将影响所有接收器输出的错误消息。

`log_timestamps`所允许的值有`UTC`（默认值）和`SYSTEM`（本地系统时区）。时间戳使用ISO 8601/RFC3339格式书写：YYYY-MM-DDThh:mm:ss.uuuuuu，再加上尾值“z”，表示Zulu时间（UTC)，或±hh:mm（表示相对于UTC的本地系统时区调整的偏移量）。比如：
```
2020-08-07T15:02:00.832521Z            (UTC)
2020-08-07T10:02:00.832521-05:00       (SYSTEM)
```

#### 5.4.2.10 错误日志文件刷新和重命名（不明白）

如果你使用`FLUSH ERROR LOGS`或`FLUSH LOGS`语句，或`mysqladmin flush-logs`命令刷新错误日志，服务器会关闭并重新打开其正在写入的任何错误日志。要重命名一个错误日志文件，请在刷新前手动重命名。刷新日志，然后用原来的文件名打开一个新的文件。比如，假设日志文件名为host_name.err，使用下面的命令重命名该文件，并创建一个新文件：
```shell
mv host_name.err host_name.err-old
mysqladmin flush-logs
mv host_name.err-old backup-directory
```
在Windows上，使用rename命令，代替mv。

如果错误日志文件的位置不可被服务器写入，则日志刷新操作创建一个新日志文件将会失败。比如，在Linux上，服务器可能会将错误日志写入/var/log/mysqld.log，/var/log目录归root用户所有，mysqld不可写入。关于处理这种情况的信息，见5.4.6节 服务器日志管理。

如果服务器为写入命名的错误日志文件，则当错误日志刷新时，不会有错误日志文件重命名。

### 5.4.3 通用查询日志

通用查询日志是mysqld所做的事情的整体记录。当客户端连接或断开连接时服务器将信息写入该日志，该日志还会记录从客户端收到的每条SQL语句。当你怀疑一个客户端出现了错误并且向清楚地知道客户端向mysqld发送了什么，通用查询日志将会非常有用。

显示客户端连接时间的每一行，该行还包括使用`using connection_type`指定用于建立连接的协议。`connection_type`是以下类型的一种：
* TCP/IP（不用SSL建立的TCP/IP连接）
* SSL/TLS（用SSL建立的TCP/IP连接）
* Socket（Unix套接字文件连接）
* Named Pipe（Windows命名管道连接）
* Shared Memory（Windows共享内存连接。

mysqld按照接收到的顺序将语句写入查询日志，可能与他们执行的顺序不一样。该记录顺序与二进制日志有差别，二进制日志写入语句的顺序是语句执行完之后但所有锁被释放之前。此外，查询日志还会包含只查询数据的语句，但这类数据不会记录到二进制日志中。

在一个复制源服务器上使用基于语句的二进制日志记录时，被他的从服务器接收到的语句会写入每个从服务器的查询日志。如果一个客户端使用mysqlbinlog实用程序读取事件并将其传给服务器，则语句会写入服务器的查询日志。

然而，当使用基于行的二进制日志记录时，更新操作以行更改的方式发送，而不是发送SQL语句，而且当`binlog_format`为`ROW`时，这些更新语句永远不会写入查询日志。当`binlog_format`设置为`MIXED`时，一个给定的更新操作也有可能不会写入查询日志，这取决于使用的语句。见17.2.1.1节 基于语句的和基于行的复制的优缺点。

默认情况下，通用查询日志是禁用的。要明确指定初始通用查询日志状态，使用`--general_log={0|1}`选项，若该选项没有参数或参数为1，则启用通用查询日志，若该选项参数为0，则禁用通用查询日志。要制定一个通用查询日志文件名，使用`--general_log_file=file_name`选项。要指定日志目的地，使用`log_output`系统变量（如5.4.1节 选择通用查询日志和慢查询日志输出目的地 中描述）。
>Note
>
如果你指定了日志目的地为table，见[Log Tables and "Too many open files"](https://dev.mysql.com/doc/refman/8.0/en/log-destinations.html#log-destinations-tables-open-files)。

如果没有为通用查询日志指定名字，默认名字为`host_name.log`。服务器会在`data`目录下创建该文件，除非使用绝对路径名指定了一个不同的目录。

要在运行时禁用或启用通用查询入职或修改日志文件名，使用`general_log, general_log_file`全局系统变量。`general_log`设置为0或OFF来禁用通用查询日志，设置为1或ON来启用通用查询日志。设置`general_log_file`来指定日志文件名。如果日志文件已经打开了，则它会关闭并打开新文件。

当通用查询日志启用时，服务器会将输出写入由`log_output`系统变量指定的任何目的地。如果启用了通用查询日志，服务器会打开该日志文件并将启动消息写入。然而，后续的查询记录不会写入日志文件，除非`log_output`选择了`FILE`日志目的地。如果目的地为`NONE`，即使启用了通用查询日志，服务器也不会写入任何查询。如果日志目的地值不包含`FILE`，设置日志文件名不会对日志记录产生影响。

服务器重启和日志刷新不会导致新的通用查询日志的产生（尽管刷新会关闭并重新打开日志文件）。要重命名日志文件并创建一个新的，使用如下命令：
```shell
shell> mv host_name.log host_name-old.log
shell> mysqladmin flush-logs
shell> mv host_name-old.log backup-directory
```
Windows上，使用rename代替mv。

你也可以在运行时通过禁用通用查询日志来为其重命名：
```sql
SET GLOBAL general_log = 'OFF';
```
在日通用查询日志志被禁用时，从外部重命名该日志文件（比如，从命令行）。然后重新启用日志：
```sql
SET GLOBAL general_log = 'ON';
```
该方法适用于任何平台，且不需要服务器重启。

要为当前会话禁用或启用通用查询日志，请设置会话的`sql_log_off`变量为ON或OFF。（这假设启用了通用查询日志本身）

写入通用查询日志的密码会被服务器重写，不会以纯文本的形式出现。通过使用`--log-raw`选项启动服务器，可以禁止通用查询日志的密码重写。该选项可以用于诊断目的，可以查看服务器接收到的准确的语句文本，但是对于生产环境的安全原因，不推荐使用此选项。

密码重写一个可能的后果是无法解析的语句不会写入通用查询日志，~~因为无法知道他们没有密码。~~需要记录所有的语句（包括错误语句）的情况应该使用`--log-raw`选项，但要记住这也会跳过密码重写。

密码重写仅在需要纯文本密码时发生。 对于具有期望密码哈希值的语法的语句，不会发生重写。 如果为此类语法错误地提供了纯文本密码，则密码将按给定方式记录，而不会重写。

`logg_timestamps`系统变量控制着写入通用查询日志文件的消息的时间戳的时区（慢查询日志可错误日志也是如此）。但它不会影响写入日志表的通用查询日志和慢查询日志消息的时区，但可以使用`CONVERT TZ()`或设置会话`time_zone`系统变量将日志表中检索的行从本机系统时区转换为任何需要的时区。

### 5.4.4 二进制日志(to be continued)
...
*5.4.4.4 mysql数据库中表的更改的日志记录格式*

*5.4.4.5 二进制日志事务压缩*

二进制日志包含描述数据库变化的事件，比如表的创建操作或表数据改变。还包含可能会导致变化的语句的事件（比如，一个没有匹配到行的`DELETE`语句），除非使用的是基于行的日志。二进制日志还会包含有关更新数据的语句用时的信息。二进制日志有两个重要目标：
* 对于复制，复制源服务器上的二进制日志提供了要发送到从服务器的数据变更记录。从服务器将包含数据变更信息的二进制日志发送到其从服务器，然后重做这些事件，与源服务器一样，做同样的数据变更。见17.2节 复制实现。
* 某些数据恢复操作需要用到二进制日志。~~恢复备份后，将重新执行备份后记录的二进制日志中的事件。这些事件使数据库从备份点开始更新。见 第 7.5 节“时间点（增量）恢复”。~~

二进制日志不用于诸如`SELECT, SHOW`等诸如不修改数据的语句。要记录所有语句（比如，识别有问题的查询），使用通用查询日志。见5.4.3节 通用查询日志。

在服务器上启用二进制日志会使性能略有下降。然而，二进制日志在搭建复制环境和恢复操作方面的好处通常超过这种轻微的性能下降。

二进制日志对于意外停止具有较强的适应性。二进制日志仅记录或读取完成的事件和事务。

写入二进制日志中的语句中的密码会被服务器重写，不会以纯文本的形式出现。见6.1.2.3节 密码和日志记录

从MySQL8.0.14开始，二进制日志和中继日志文件可以被加密，帮助保护这些文件和所包含的潜在的敏感数据不被外部攻击者滥用，也防止操作系统用户在未经授权的情况下查看他们。你可以在MySQL服务器上通过将`binlog_encryption`系统变量设置为ON开启加密。更多信息，见17.3.2节 加密二进制日志文件和中继日志文件。

下面的内容介绍能够影响二进制日志操作的服务器选项和变量。完整列表见17.1.6.4节 二进制日志选项和变量。

#### 5.4.4.1 二进制日志格式

服务器使用几个记录格式来记录二进制日志中的信息。
* MySQL中的复制功能最初是基于SQL语句从源服务器向从服务器的传播。这被称为基于语句的日志记录。你可以通过`--binlog-format=STATEMENT`选项启动服务器，来设置这种格式的二进制日志。
* 在基于行的日志记录中（默认），源服务器将事件写入二进制日志，表明表中的行是如何改变的。你可以通过`--binlog_format=ROW`选项启动服务器，来设置这种格式的二进制日志。
* 还有第三种选项：混合日志记录。对于混合日志记录，基于语句的日志记录是默认的，但在下面介绍的几种情况下日志记录模式会自动切换为基于行的日志记录。你可以使用`--binlog-format=MIXED`选项启动mysqld，使MySQL使用混合日志记录。

日志记录格式也可以由正在使用的存储引擎设置或限制，这有助于消除在使用不同存储引擎的源服务器和从服务器之间复制某些语句时出现的问题。

对于基于语句的复制，复制非确定性语句时可能会出现问题。~~对于决定一个给定的语句对基于语句的复制来说是否安全，MySQL根据它是否能保证语句可以用基于语句的日志记录来复制  来确定。~~如果MySQL不能保证，它将该局域标记为潜在的不可靠，并发出警告：Statement may not be safe to log in statement format。

你可以使用基于行的复制来避免这些问题。

#### 5.4.4.2 设置二进制日志格式（to be continued）

#### 5.4.4.3 混合二进制日志记录格式（to be continued）

#### 5.4.4.4 mysql数据库中表的更改的日志记录格式

`mysql`数据库中权限表的内容可以直接修改（比如，使用insert或delete），也可以间接修改（比如，使用grant或create user）。影响`mysql`数据库的语句按照以下规则写入二进制日志：
* 可以改变`mysql`数据库数据的数据操纵语句根据`binlog_format`系统变量的设置直接记录到二进制日志中。这适用于`INSERT, UPDATE, DELETE, REPLACE, DO, LOAD DATA, SELECT, TRUNCATE TABLE`语句。
* 间接改变`mysql`数据库的语句，会忽略`binglog_format`的设置，直接将语句记录到二进制日志中。

`CREATE TABLE...SELECT`语句是数据定义和数据操纵的组合。`CREATE TABLE`部分使用语句格式记录，`SELECT`部分根据`binlog_format`的值进行记录。

#### 5.4.4.5 二进制日志事务压缩（to be continued）

### 5.4.5 慢查询日志

慢查询日志由执行时间超过`long_query_time`，并且审查的行数不少于`min_examined_row_limit`的语SQL语句组成。慢查询日志用于找出执行时间长的检索，以便进行优化。但是，检查长的慢查询日志是一项耗时的任务。为了方便，可以使用`mysqldumpslow`命令处理慢查询日志文件并总结其内容。见4.6.10节 mysqldumpslow—总结慢查询日志文件。

获取初始锁的时间不计入执行时间。在一条语句执行完并释放掉所有锁之后，mysqld才会将其写入慢查询日志，所以日志中语句的顺序可能会与执行顺序不同。

*5.4.5.1 慢查询日志参数*

*5.4.5.2 慢查询日志内容*

#### 5.4.5.1 慢查询日志参数（to be continued）

`long_query_time`的最小值和默认值分别是0和10秒。该值可以指定为微妙。

默认情况下，慢查询日志不会记录管理语句，也不会记录那些不使用索引的检索。该行为可以使用`log_slow_admin_statements, log_queries_not_using_indexes`改变，后面将会介绍。

默认情况下，慢查询日志是禁用的。使用`--slow_query_log[={0|1}]`可以指定慢查询日志的初始状态。没有参数或参数为1，表示启用慢查询日志，参数为0，表示禁用慢查询日志。要指定日志文件名，使用`--solw_query_log_file=file_name`。要指定目的地，使用`log_output`系统变量（见5.4.1节 选择通用查询日志和慢查询日志的输出目的地）。
>Note
>
如果你指定`TABLE`为目的地，见[`Log Tables and "Too many open files"Errors`](https://dev.mysql.com/doc/refman/8.0/en/log-destinations.html#log-destinations-tables-open-files)。

如果你没有为慢查询日志指定名字，则默认日志文件名为`host_name-slow.log`。服务器在`data`目录下创建该文件，除非用绝对路径名指定了一个不同的目录。

要在运行时禁用或启用慢查询日志，或更改日志文件名，使用全局`slow_query_log, slow_suery_log_file`系统变量。设置`slow_query_log`为0是禁用，1是启用。设置`slow_query_log_file`指定日志文件名。如果日志文件已经打开了，则会被关闭并打开新文件。

如果使用`--log-short-format`选项，则服务器向慢查询日志写入更少的信息。

### 5.4.6 服务器日志维护

如5.4节 MySQL服务器日志 中介绍，MySQL服务器可以创建几种不同的日志文件来帮助你查看正在发生的活动。但是，你必须经常清理这些文件，以确保日志文件不会占据太多的磁盘空间。

当启用MySQL日志时，你可能希望不时备份并移除旧的日志文件，并告诉MySQL将日志记录到新的文件里。见7.2节 数据库备份方法。

在一个Linux（Red Hat）安装中，你可以使用`mysql-log-rotate`脚本进行日志维护。如果你是从RPM发行版安装MySQL，该脚本应该已经自动安装。如果你将二进制日志用于复制，你要谨慎的使用此脚本。你不应该移除二进制日志，除非你确认日志中的所有内容已经被从服务器处理。

在其他系统上，你必须安装一个从`cron`（或其等效应用）启动的简短脚本用于处理日志文件。

在服务器的二进制日志到期后，二进制日志文件会被自动移除。移除操作可以在服务器启动时或二进制日志刷新时进行。默认二进制日志有效期为30天。要指定可选的有效期，使用`binlog_expire_logs_seconds`系统变量。如果你正在使用复制，你应该指定一个不低于你的从服务器可能会落后于源服务器的最长时间的有效期。若要移除二进制日志，使用`PURGE BINARY LOGS`语句（见13.4.1.1 OURGE BINARY LOGS语句）。

要强制MySQL用新的日志文件启动，刷新日志。日志刷新可使用`FLUSH LOGS, mysqladmin flush-logs, mysqladmin refresh, mysqldump --flush-logs, mysqldump --master-data`命令。见13.7.8.3节 FLUSH语句，4.5.2节 mysqladmin—一个MySQL服务器管理程序，4.5.4节 mysqldump—一个数据库备份程序。另外，当当前二进制日志大小达到了`max_binlog_size`系统变量的值，服务器会自动刷新二进制日志。

`FLUSH LOGS`支持可选的修饰符，以便能够选择刷新单个日志（比如`FLUSH BINART LOGS`）。见13.7.8.3节 FLUSH语句。

日志刷新操作有以下影响：
* 如果启用了二进制日志，服务器会关闭当前二进制日志并使用下一个序号打开一个新的日志文件。
* 如果通用查询日志或慢查询日志记录到了一个日志文件中，则服务器会关闭并重新打开该日志文件。
* 如果服务器用`--log-error`选项启动，会导致错误日志写入一个文件中，服务器会关闭并重新打开该日志文件。

日志刷新语句或命令的执行需要连接到服务器的用户拥有`RELOAD`权限。在Unix和类Unix系统上，另一种刷新日志的方式是向服务器发送一个信号，这可以由root或拥有服务器进程的账户完成。（见4.10节 MySQL中的Unix信号处理。）不用连接至服务器信号即可执行日志刷新：
* `SIGHUP`信号刷新所有的日志。但是`SIGHUP`信号还有其他的不受欢迎影响。
* 从MySQL8.0.19开始，`SIGUSR1`信号会导致服务器刷新错误日志、通用查询日志、慢查询日志。如果你只对刷新这三种日志感兴趣，`SIGUSR1`信号作为一个更轻量级的信号，并且没有`SIGHUP`产生的对日志不相关的影响，可以其对三种日志进行刷新。

如之前提到的，刷新二进制日志会创建一个新的二进制日志文件，然而刷新通用查询日志、慢查询日志、错误日志仅仅是关闭并重新打开日志文件，对于后三者，要在Unix上创建新的日志文件，先重命名当前日志文件，然后再刷新它。在刷新时，服务器会用之前的文件名打开新的日志文件。比如，如果通用查询日志、慢查询日志、错误日志文件名为`mysql.log, mysql-slow.log, err.log`，你可以在命令行中使用下面的命令：
```shell
...
mysqladmin flush-logs
```
在Windows上，使用rename代替mv。

这时，你可以备份`mysql.log.old, mysql-slow.log.old, err.log.old`，然后将其从磁盘移除。

要重命名通用查询日志和慢查询日志，首先连接到服务器并禁用该日志：
```sql
SET GLOBAL general_log = 'OFF';
SET GLOBAL slow_query_log = 'OFF';
```
禁用后，从外部重命名日志文件（比如，从命令行）。然后重新启用日志：
```sql
SET GLOBAL general_log = 'ON';
SET GLOBAL slow_query_log = 'ON';
```
>Note
>对于在外部重命名日志文件后服务器给定的日志文件，该文件的位置对于服务器来说必须是可写的。但可能并不总是这种情况。比如，Linux系统上，服务器可能将错误日志写入/var/log/mysqld.log，但/var/log归root用户拥有，mysqld不可写。这种情况，日志刷新操作无法创建新的日志文件。
>
要处理这种情况，你必须在重新命名初始日志文件后创建具有正确所有权的心日志文件。比如，以root身份执行以下命令：
	```shell
...
MySQL服务器有一个基于组件的用于扩充服务器功能的基础设施。~~组件提供服务器和其他组件可获得的服务。~~（就服务使用而言，服务器也是一个组件，与其他组件一样。）组件仅通过他们提供的服务进行交互。

MySQL发行版包括实现服务器扩展的组件：
* 配置错误日志组件。见5.4.2节 错误日志，5.5.3节 错误日志组件。
* 用于检查密码的组件。见6.4.3节 密码验证组件。
* 密钥环组件，为敏感信息提供安全存储。见6.4.4节 MySQL密钥环。
* 能够使应用程序将他们的消息事件添加到审查日志的组件。见6.4.6节 审查消息组件。
* 实现用于访问查询属性的可加载函数的组件。见9.6节 查询属性。

由一个组件实现的系统变量和状态变量在组件被安装时~~曝光~~，并且名字以特定于组件的前缀开头。比如，`log_filter_dragnet`错误日志过滤器组件实现了一个名为`log_error_filter_rules`的系统变量，其全名为`dragnet.log_error_filter_rules`。要引用次变量，应使用全名。

下面的章节介绍如何安装和卸载组件，如何查看运行时哪些组件被安装了以及如何获取他们的信息。

对于组件内部实现的信息，参见MySQL服务器的Doxygen文档，可在[https://dev.mysql.com/doc/index-other.html](https://dev.mysql.com/doc/index-other.html) 获得。比如，如果你打算写你自己的组件，这里的信息对于理解组件是如何工作的很重要。

### 5.5.1 安装和卸载组件

组件在使用之前必须先加载到服务器。MySQL支持运行时手动组件加载，和服务器重启期间自动加载。当一个组件被加载后，有关它的信息就是可获得的，如5.5.2节 获取组件信息 所述。

`INSTALL COMPONENT, UNINTSALL COMPONENT`SQL语句用于组件的加载和卸载。比如：
```sql
INSTALL COMPONENT 'file://component_validate_password';
UNINSTALL COMPONENT 'file://component_validate_password';
```

加载器服务处理组件加载和卸载，同时将加载的组件登记到`mysql.component`系统表。

操作组件的SQL语句以以下方式影响服务器操作和`mysql.component`系统表：
* `INSTALL COMPONENT`语句向服务器加载组件。该组件立刻变为活动状态。加载器服务还会将加载的组件登记到`mysql.component`系统表。对于后续的服务器重启，服务器会在启动序列期间加载所有列在`mysql.component`表中的组件，即使服务器用`--skip-grant-tables`选项启动时也是如此。
* `UNINSTALL COMPONENT`语句是组件停止工作并将其从服务器中卸载。加载器服务也会从`mysql.component`表中删除登记信息，以便在后续的重启中，在启动序列期间不再加载这些组件。

与用于服务器插件的相应的`INSTALL PLUGIN`语句相比，用于组件的`INSTALL COMPONENT`语句提供了一个最重要的优点，就是不需要知道任何特定于平台的文件名后缀，用于命名该组件。这意味着一个给定的`INSTALL COMPONENT`语句可以跨平台同样执行。

一个组件被安装时，可能会自动安装相关的可加载函数。如果是这样，则该插件在卸载时也会自动卸载这些函数。

### 5.5.2 获取组件信息

`mysql.component`系统表包含当前加载的组件的信息，并且会显示使用`INSTALL COMPONENT`语句登记的组件。从该表中检索显示哪些组件被安装了，比如：
```sql
...
+--------------+--------------------+------------------------------------+
```
`component_id, component_group`的值是为了内部使用。`component_urn`是`INSTALL COMPONENT, UNINSTALL COMPONENT`语句中用于加载和卸载插件的URN（同意资源名称）。

### 5.5.3 错误日志组件（to be continued）

### 5.5.4 查询属性组件

从MySQL8.0.23开始，一个组件服务提供查询属性的访问（见9.6节 查询属性）。`query_attributes`组件使用此服务来提供对 SQL 语句中的查询属性的访问。
* 目的：实现`mysql_query_atrribute_string()`函数接受属性名称参数并以字符串形式返回属性值，如果属性不存在，就返回NULL。
* URN：`file://component_query_attributes`

想要合并使用`query_attributes`的相同的查询属性组件服务的开发者，应该查阅MySQL源代码的`mysql_query_attributes.h`文件。
...
MySQL支持允许创建服务器插件的插件API。插件可以在服务器启动时加载，也可以在运行时加载和卸载，不用重启服务器。该接口支持的插件包括但不限于：存储引擎，`INFORMATION_SCHEMA`表，全文解析插件和服务器扩展。

MySQL发行版包括几个实现服务器扩展的插件：
* 用于验证客户端尝试连接到MySQL服务器的插件。插件可用于多种身份验证协议。见6.2.17节 可插拔身份验证。
* 连接控制插件，使管理员能够在一定数量的连续客户端连接尝试失败后引入增加的延迟。见6.4.2节 连接控制插件。
* 密码验证插件，实施密码强度策略并评估潜在密码的强度。见6.4.3节 密码验证组件。
* 半同步复制插件，实现了一个复制功能接口，只要有一个从服务器响应了全部的事务，就允许源服务器继续进行。
* 组复制使你可以通过一组MySQL服务器实例创建高可用的分布式MySQL服务，并能保证数据一致性、冲突检测和解决、组成员服务全部内建。见第十八章 组复制。
* MySQL企业版包括一个线程池插件，该插件管理连接线程，通过有效管理大量客户端连接的语句执行线程，来提高服务器性能。
* MySQL企业版包括一个审计插件，用于监视并记录连接和查询活动。见6.4.5节 MySQL企业版审计。
* MySQL企业版包括一个防火墙插件，~~该插件实现了应用程序级防火墙，使数据库管理员能够根据与已接受语句模式的许可列表的匹配来允许或拒绝 SQL 语句执行。~~见6.4.7节 MySQL企业版防火墙。
* 查询语句重写插件检查MySQL服务器收到的语句，并可能在服务器执行该语句之前重写他们。见5.6.4节 重写器查询重写插件，5.6.5节 ddl_rewriter插件。
* 版本令牌，支持创建可同步服务器令牌，应用程序可以使用这些令牌阻止访问不正确的或过时的数据。版本令牌基于一个插件库，该插件库实现了一个`version_tokens`插件和一组可加载的函数。见5.6.6节 版本令牌。
* 密钥环插件，提供敏感信息的安全存储。见6.4.4节 MySQL密钥环
~~在MySQL8.0.24中，MySQL密钥环开始从插件过渡到使用组件基础设置，促进使用名为`daemon_keyring_proxy_plugin`的插件成为了插件和组件服务API之间的桥梁。~~见5.6.8节 密钥环代理桥接插件。
* X插件扩充了MySQL服务器，使其能够用作文档存储。运行X插件使MySQL服务器能够使用X协议与客户端通信，~~该协议旨在将MySQL的ACID兼容存储能力公开为文档存储。~~见20.5节 X插件。
* 克隆允许从一个本地或远程MySQL服务器实例克隆InnoDB数据。见5.6.7 克隆插件。
* 测试框架插件，特使服务器的服务。关于这些插件的信息，见MySQL Server Doxygen文档的Plugins for Testing Plugin Services部分，可在[https://dev.mysql.com/doc/index-other.html](https://dev.mysql.com/doc/index-other.html) 获取。

下面的章节介绍如何安装和卸载插件，以及如何查看运行时哪些插件被安装了，如何获取他们的信息。关于编写插件的信息，见[The MySQL Plugin API](https://dev.mysql.com/doc/extending-mysql/8.0/en/plugin-api.html)。

### 5.6.1 安装和卸载插件（to be continued）

### 5.6.2 获取服务器插件信息（to be continued）

### 5.6.3 MySQL企业版线程池（to be continued）

### 5.6.4 重写器查询重写插件（to be continued）

### 5.6.5 ddl_rewriter插件（to be continued）

### 5.6.6 版本令牌（to be continued）

### 5.6.7 克隆插件（to be continued）

### 5.6.8 密钥环代理桥接插件（to be continued）

### 5.6.9 MySQL插件服务（to be continued）
...
MySQL支持可加载函数，即不是内建的函数，但是可以在运行时（以及在启动期间或之后）加载的函数，以扩展服务器功能；或卸载，以移除服务器功能。有关描述可用可加载函数的表，见12.2节 可加载函数参考。可加载函数与内建（原生）函数相比，后者作为服务器的一部分被实现，并总是可用的，有关表格，见12.1节 内建函数和操作符参考。
>Note
>
可加载函数之前被称为用户自定义函数（UDFs），这个用词不是很恰当，因为“user-defined”还可以用于其他类型的函数，比如存储函数（一种使用SQL语句编写的存储对象），和通过修改没服务器源代码添加的原生函数。

MySQL发行版包括实现以下全部或部分服务器功能的可加载函数：
* 组复制使你可以通过一组MySQL服务器实例创建高可用的分布式MySQL服务，并能保证数据一致性、冲突检测和解决、组成员服务全部内建。见第十八章 组复制。
* MySQL企业版包括基于OpenSSL库的执行加密操作的函数。见6.5.1节 MySQL企业版加密。
* MySQL企业版包括用于数据屏蔽和去标识化操作的提供SQL级API的函数。见6.5.1节 MySQL企业版数据屏蔽和去标识化元素。
* MySQL企业版包括用于监测和记录连接和查询活动的审计日志记录。见MySQL企业版审计，6.4.6节 审计消息组件。
* MySQL企业版包括一个防火墙功能，它实现了一个应用级别的防火墙，~~使得数据库管理员可以根据接受到语句的模式匹配，来允许或禁止SQL语句的执行。~~见6.4.7节 MySQL企业版防火墙。
* 一个查询重写器检查MySQL服务器收到的语句，然后服务器在执行他们之前可能会先重写他们。见5.6.4节 重写器查询重写插件。
* 版本令牌支持创建和同步服务器令牌，应用程序可以用该令牌阻止访问不正确的和过期的数据。见5.6.6节 版本令牌。
* MySQL密钥环提供敏感信息的安全存储。见6.4.4节 MySQL密钥环。
* 锁服务为应用程序提供了一个锁接口。见5.6.9.1节 锁服务。
* 一个函数提供了查询属性的访问。见9.6节 查询属性。

下面的章节描述如何安装和卸载可加载函数，如何查看运行时哪些可加载函数被加载了以及如何获取他们的信息。

有些情况下，一个可加载函数通过安装实现了该函数的组件来加载，而不是通过直接加载该函数。关于特定可加载函数的详细信息，请查阅包含该功能的服务器功能的安装说明。

有关编写可加载函数的信息，见[Adding Functions to MySQL](https://dev.mysql.com/doc/extending-mysql/8.0/en/adding-functions.html) 。

### 5.7.1 安装和卸载可加载函数（to be continued）

### 5.7.2 获取可加载函数的信息

`performance_schema.user_defined_functions`表包含最近安装的可加载函数的信息：
```sql
SELECT * FROM performance_schema.user_defined_functions;
```
`mysql.func`系统表也列出了已安装的可加载函数，但只列出了使用`CREATE FUNCTION`语句安装的函数。`performance_schema.user_defined_functions`表同时列出了使用`CREATE FUNCTION`语句安装的可加载函数和通过组件和插件自动安装的可加载函数。这种差异使得`performance_schema.user_defined_functions`比`mysql.func`更适合与查看安装的可加载函数。见27.12.21.8节 user_defined_functions表。

## 5.8 在一台机器上运行多个MySQL实例（to be continued）

*5.8.1 设置多个数据目录*

*5.8.2 在Windows上运行多个MySQL实例*

*5.8.3 在Unix上运行多个MySQL实例*

*5.8.4 在多服务器环境中使用客户端程序*

有些情况下，你可能想要在一台机器上运行多个MySQL实例。你可能想在不干扰现有生产设置的同时测试一个新的MySQL版本。或者你可能想让不同的用户访问他们自己管理的不同的mysqld服务器。（比如，你可能是一个网络服务提供者，想为不同的客户提供独立的MySQL安装。）

可以为每个实例使用不同的MySQL服务器二进制文件，或者为多个实例使用相同的二进制文件，或者这两种方式的任意组合。比如，你可能运行了一个MySQL5.7的服务器和一个MySQL8.0的服务器，想要查看不同的版本如何处理一个给定的工作负载。或者你可能在当前生产版本运行了多个服务器，每个实例管理不同的数据库集合。

无论你是否使用不同的服务器二进制文件，你运行的每个实例必须为多个操作参数配置唯一值。这消除了实例之间潜在的冲突。参数可以在命令行中配置，在选项文件中配置，或通过设置环境变量配置。见4.2.2节 指定程序选项。要查看一个给定的实例使用的选项值，连接到该实例然后执行`SHOW VARIABLES`语句。

MySQL实例管理的主要的资源是`data`数据目录。每个实例应该用不同的数据目录，用`--datadir=dir_name`选项指定该目录的位置。有关使用自己的数据目录配置每个实例的方法，以及有关不这样做的危险警告，见5.8.1节 设置多个数据目录。

除了使用不同的数据目录外，几个其他的选项的值对于每个服务器实例来说必须是不同的：
* --port=port_num:
--port控制TCP/IP连接的端口号。或者，如果主机有多个网络地址，你可以通过设置`bing_address`系统变量让每个服务器监听不同的地址。
* --socket={file_name | pipe_name}
--socket在Unix上控制Unix套接字文件路径，在Windows上控制命名管道的名字。在Windows上，仅为那些配置为允许命名管道连接的服务器指定不同的管道名字是必要的。
...
在考虑MySQL安装中的安全性时，你应该考虑各种可能的主题，以及他们是如何影响MySQL服务器和相关应用程序的安全的：
* 影响安全的一般因素。这包括选择好的密码，不将不需要的权限授权给用户，通过阻止SQL注入和数据损坏确保应用程序安全性，以及其他。见6.1节 一般安全问题。
* 安装本身的安全。数据文件，日志文件以及你的安装的所有应用程序文件都应被保护，以确保这些文件对于未经授权的用户是不可读的和不可写的。见2.10节 安装后的设置和测试。
* 数据库系统本身的访问控制和安全，包括被授予访问数据库、视图和存储程序的权限的用户和数据库。更多信息，见6.2节 访问控制和账户管理。
* 安全相关的插件提供的功能。见6.4节 安全组件和插件。
* MySQL和你的系统的网络安全。安全性与对单个用户的授权有关，但你可能还希望限制 MySQL，使其仅在 MySQL 服务器主机上本地可用，或对一组有限的其他主机可用。
* 确保你有足够的和适当的数据库文件、配置文件和日志文件的备份。还要确保有一个适当的恢复解决方案，并测试是否能够从备份中成功恢复信息。见第七章 备份和恢复。
>Note
>
本章中的几个主题也在[Secure Deployment Guide](https://dev.mysql.com/doc/mysql-secure-deployment-guide/8.0/en/) 中进行了讨论，该指南提供了部署 MySQL Enterprise Edition Server 的通用二进制分发版的过程，其中包含管理 MySQL 安装安全性的功能。
